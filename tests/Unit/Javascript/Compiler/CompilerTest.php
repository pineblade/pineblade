...
{
    return app(Compiler::class)->compileString("<?php $input;");
}

test('must compile statement', function (string $php, string $js) {
    expect(compile($php))->toBe($js);
})->with('syntax-features');

test('traits are disabled', function () {
    expect(fn() => compile('new class () {use Foo;}'))->toThrow(UnsupportedSyntaxException::class);
});
