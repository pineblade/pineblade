...
readonly class Compiler
{
    ...
    public function compileNode(Node $node, bool $varAccess = false): string
    {
        switch (get_class($node)) {
            ...
            case Property::class:
                {
                    ...
                    if ($this->hasInjectValue($node)) {
                        $default = $this->injectValueProcessor->process($node, $this);
            ...
            case Node\Stmt\Function_::class:
                {
                    ...
                    return Scope::be($currentMethod, function () use ($node) {
                        ...
                        if ($node instanceof Node\Stmt\ClassMethod && $this->hasAttributes($node, 'Server')) {
                            ...
                            $methodBody[] = "{$node->name->name}(...args) {{$this->serverMethodCompiler->compile(
                                $node,
                            )}}";
                            ...
                            $methodBody[] =
                                '(' . implode(', ', $methodParams) . ") => {$this->compileNode($node->expr, true)}";
                        }
            ...
            case Node\Expr\MethodCall::class:
                {
                    ...
                    if ($node instanceof Node\Expr\FuncCall) {
                        if (
                            $node->name instanceof Node\Expr\Closure
                            || $node->name instanceof Node\Expr\ArrowFunction
                        ) {
            ...
            case Node\Stmt\If_::class:
                {
                    $statement = [
                        "if ({$this->compileNode($node->cond)}) {{$this->compileNodesWithinLocalScope($node->stmts)}}",
                    ];
            ...
            case Node\Stmt\While_::class:
                {
                    return "while({$this->compileNode(
                        $node->cond,
                        true,
                    )}) {{$this->compileNodesWithinLocalScope($node->stmts)}}";
                }
            case Node\Stmt\Do_::class:
                {
                    return "do {{$this->compileNodesWithinLocalScope($node->stmts)}} while({$this->compileNode(
                        $node->cond,
                        true,
                    )})";
                }
            ...
            case Node\Stmt\TryCatch::class:
                {
                    ...
                    foreach ($node->catches as $catch) {
                        $parts[] = "catch ({$this->compileNode(
                            $catch->var,
                            true,
                        )}) {{$this->compileNodes($catch->stmts)}}";
                    }
                    if ($node->finally) {
    ...
    private function hasAttributes(
        Node\Expr\ArrowFunction|Node\Expr\Closure|Node\Stmt\ClassMethod|Node\Stmt\Function_|Property $node,
        string $name,
    ): bool {
    ...
    public function compileNodesWithinLocalScope(
        array $nodes,
